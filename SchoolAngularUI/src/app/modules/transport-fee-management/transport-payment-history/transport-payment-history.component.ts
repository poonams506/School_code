...
import { AcademicYear, DatatableResponseModel, FeePaymentSelectDto, FeePaymentServiceProxy, MasterServiceProxy, TransportFeePaymentSelectDto, TransportFeePaymentServiceProxy } from 'src/app/services/school-api-service';
...
@Component({
  selector: 'app-transport-payment-history',
  templateUrl: './transport-payment-history.component.html',
  styleUrls: ['./transport-payment-history.component.scss']
})
export class TransportPaymentHistoryComponent implements OnInit {
  ...
  gradeId : number;
  divisionId : number;
  fetchedInfo : TransportFeePaymentSelectDto;
  ...
  getStudentFeePayment(){
    if(this.consumerId && this.consumerId>0 && this.academicYearId && this.academicYearId > 0){
      this.feePaymentService.getTransportFeePaymentSelect(this.academicYearId, this.consumerId,this.roleId,this.transportConsumerStoppageMappingId).
      ...
      ajax: (requestListModel: any, callback : any) => {
        that.http
          .post<DatatableResponseModel>(
            environment.API_BASE_URL+"/api/TransportFeePayment/GetTransportFeePaymentHistoryGridList",
            {getListModel:requestListModel,academicYearId:this.academicYearId, roleId : this.roleId,
              consumerId:this.consumerId,transportConsumerStoppageMappingId:this.transportConsumerStoppageMappingId},{}
          ).subscribe(resp => {
  ...
  viewHistory(item : any){
    ...
    modalRef.componentInstance.transportConsumerStoppageMappingId=this.transportConsumerStoppageMappingId;
    modalRef.componentInstance.transportFeePaymentId=item.transportFeePaymentId;
  ...
  clearCheque(item : any){
    ...
    newConfirmBox.openConfirmBox$().subscribe((resp:any) => {
     if(resp?.success){
      this.feePaymentService.transportClearCheque(item.transportFeePaymentId).
  ...
  deleteFeePayment(item : any){
    ...
    newConfirmBox.openConfirmBox$().subscribe((resp:any) => {
     if(resp?.success){
      this.feePaymentService.transportFeePaymentDelete(item.transportFeePaymentId).
