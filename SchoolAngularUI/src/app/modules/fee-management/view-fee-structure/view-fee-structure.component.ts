...
@Component({
  ...
  styleUrls: ['./view-fee-structure.component.scss']
})
export class ViewFeeStructureComponent implements OnInit{
      ...
      ajax: (requestListModel: any, callback : any) => {
        that.http
          .post<DatatableResponseModel>(
            environment.API_BASE_URL+"/api/FeeParticular/GetFeeParticularGridList",
...
this.router.navigate(['fee-management/create-fee-structure',encryptedString]);
}
...
publishFeeParticular(gradeId:number,divisionId:number){
  ...
  newConfirmBox.openConfirmBox$().subscribe((resp:any) => {
   if(resp?.success){
    ...
    this.particularService.publishUnpublishGradeDivisionParticulars(requestDto).subscribe(result=>{
...
cloneFeeParticular(item:any){
  debugger;
  const modalRef = this.modalService.open(CloneFeeParticularStructureComponent, { size: 'md',backdrop:'static' });
...
confirmFeeParticularDelete(gradeId:number,divisionId:number) {
  ...
  newConfirmBox.openConfirmBox$().subscribe((resp:any) => {
   if(resp?.success){
    this.particularService.feeParticularDelete(gradeId,divisionId,this.academicYearId).subscribe(data=>{
...
copyFeeParticularsFromLastAY(gradeId:number,divisionId:number) {
  ...
  newConfirmBox.openConfirmBox$().subscribe((resp:any) => {
   if(resp?.success){
    this.particularService.copyFeeParticularsFromLastAY(gradeId,divisionId).subscribe(data=>{
    if(data == 1){
