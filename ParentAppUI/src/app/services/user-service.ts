...
import { AuthServiceProxy, Module, UserRoleModulePermissionDto } from './school-api-service';
...
export class UserService {
  ...
  private currentSibling:number;
  private currentSiblingClass:number;
  constructor(private authServiceProxy:AuthServiceProxy) {}


  getUser(isReload=false): Observable<UserRoleModulePermissionDto|undefined> {
 ...
 // Check if the current user has a specific permission
  ...
  getAcademicYear():Observable<number|undefined>{
 ...
 setAcademicYear(academicYearId:number){
  ...
  set CurrentUserRoleId(roleId:string){
    this.currentRole=roleId;
    localStorage.setItem('id',roleId);
  }

  get CurrentUserRoleId():string{
    if(this.currentRole){
  ...
  set CurrentSiblingClassId(classId:number){
