      ...
      ajax: (requestListModel: any, callback : any) => {
        that.http
          .post<DatatableResponseModel>(
            environment.API_BASE_URL+"/api/FeePayment/GetFeePaymentGridList",
...
setPage(){
  this.dtElement.dtInstance.then((dtInstance: any) => {
    if(sessionStorage.getItem('academic-payment-page-number') && sessionStorage.getItem('academic-payment-page-number') != null
    && sessionStorage.getItem('academic-payment-page-number') != undefined &&
    sessionStorage.getItem('academic-payment-page-number') != ""){
      if(parseInt(sessionStorage.getItem('academic-payment-page-number')?.toString()!)){
        dtInstance.page(parseInt(sessionStorage.getItem('academic-payment-page-number')?.toString()!)).draw('page');
      }
    }
    if(sessionStorage.getItem('academic-payment-search') && sessionStorage.getItem('academic-payment-search') != null
    && sessionStorage.getItem('academic-payment-search') != undefined &&
    sessionStorage.getItem('academic-payment-search') != ""){
      if(sessionStorage.getItem('academic-payment-search')?.toString()!){
        dtInstance.search(sessionStorage.getItem('academic-payment-search')?.toString()!).draw();
      }
    }
  });
  if(sessionStorage.getItem('academic-payment-student-id') && sessionStorage.getItem('academic-payment-student-id') != null
    && sessionStorage.getItem('academic-payment-student-id') != undefined &&
    sessionStorage.getItem('academic-payment-student-id') != ""){
      if(parseInt(sessionStorage.getItem('academic-payment-student-id')?.toString()!)){
        this.hlighlightedStudentId = parseInt(sessionStorage.getItem('academic-payment-student-id')?.toString()!);
      }
    }
...
getLocalStorageFilter(){
  let storedClassId = sessionStorage.getItem("PaymentListing");
...
search(){
  ...
  if (selectedClassMapping) {
    ...
    sessionStorage.setItem('PaymentListing', selectedClassId + "_" + this.gradeId + "_" + this.divisionId);
  }else {
    // If the selected class mapping is not found, set both gradeId and divisionId to null
    this.gradeId = null;
    this.divisionId = null;
    sessionStorage.removeItem('PaymentListing');
    sessionStorage.removeItem('academic-payment-page-number');
    sessionStorage.removeItem('academic-payment-student-id');
    sessionStorage.removeItem('academic-payment-search');
  ...
  this.rerender();
    ...
    pay(item : any){
      ...
      this.dtElement.dtInstance.then((x)=>{
        ...
        this.router.navigate(['fee-management/fee-payment',encryptedString]);
              });
